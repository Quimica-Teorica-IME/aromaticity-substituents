...
q_columns = ['Q2(zz_origin)']
#q_columns = ['Total |Q2|(ring atoms)', 'Total Q2(zz_ring atoms)', '|Q2|origin', 'Q2(zz_origin)', '|Q2|(1)', 'Q2(1)zz']

# Create directory to save graphs
output_dir = "graphs_hammettxQ_ML"
...
for sigma in sigma_columns:
    for q in q_columns:
        if sigma in data.columns and q in data.columns:
            subset = data[['-X', sigma, q]].dropna()  # Include the -X column in the subset
            if not subset.empty:
                ...
                clustering = DBSCAN(eps=0.7, min_samples=3).fit(subset[[sigma, q]])  # DBSCAN with adjusted eps and min_samples
                labels = clustering.labels_
                unique_labels = np.unique(labels)

                # Create a folder for each q_column inside the main output directory
                q_cleaned = sanitize_filename(q)  # Clean the q_column name for use as folder name
                q_output_dir = os.path.join(output_dir, q_cleaned)
                os.makedirs(q_output_dir, exist_ok=True)  # Create the folder if it doesn't exist
                ...
                for label in unique_labels:
                    ...
                    if label == -1:  # If DBSCAN marks as "noise"
                        label = 'Noise'
                    plt.scatter(cluster_data[sigma], cluster_data[q], label=f'Cluster {label+1}' if label != 'Noise' else label, edgecolor='black', alpha=0.7, s=400)  # Add each cluster with a different color
                plt.xlabel(sigma, fontsize=25)  # Increase the size of the X-axis label
                plt.ylabel(q, fontsize=25)  # Increase the size of the Y-axis label
                ...
                if r2 >= 0.6:
                    ...
                    if not (sigma in ['σp', 'σp-', 'σp0', 'σI'] and q == 'Qzz origin'):
                        plt.text(0.65, 0.05, equation_text, transform=plt.gca().transAxes, fontsize=20,
                ...
                plt.savefig(os.path.join(q_output_dir, scatter_filename))
                plt.close()

print(f"Scatter plots generated and saved in the '{output_dir}' folder, organized by q_columns.")
